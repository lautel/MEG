...
deepspeed --include localhost:0,1,2,3 --master_port 29503 train_meg_mistral_last_layer.py > ${logging_file} 2>&1  \
...
deepspeed --include localhost:0,1 --master_port 29501 eval_meg_mistral_last_layer.py > ${logging_file} 2>&1 \
--deepspeed ds/deepspeed_config_s2.json \
--task ${DATASET} \
--mapping_type ${mapping_type} \
--model_name_or_path ${model} \
