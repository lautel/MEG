...
CUDA_VISIBLE_DEVICES=4,5,6,7 torchrun --nproc_per_node=4 --master_port=9777 train_meg_llama.py > ${logging_file} 2>&1  \
--task ${DATASET} \
...
CUDA_VISIBLE_DEVICES=6,7 torchrun --nproc_per_node=2 --master_port=9777 eval_meg_llama.py > ${logging_file} 2>&1 \
--task ${DATASET} \
--mapping_type ${mapping_type} \
--max_num_embeds 3 \
--model_name_or_path ${model} \
