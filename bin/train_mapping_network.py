...
def main():
    parser = transformers.HfArgumentParser(
        (DataArguments, SFTConfig, ModelArguments)
    )
    ...
    def train_transformer(data_args, training_args, model_args):
        ...
        train_sampler = MinTwoLabelBatchSampler(train_dataset, batch_size=training_args.per_device_train_batch_size)
        train_dataloader = DataLoader(train_dataset, batch_sampler=train_sampler)
     
        optimizer = torch.optim.AdamW(mapper.parameters(), lr=training_args.learning_rate)
        steps_in_one_epoch = len(train_dataloader)
        batches_in_one_epoch = steps_in_one_epoch // training_args.per_device_train_batch_size
        ...
        for epoch in range(int(training_args.num_train_epochs)):
            for batch in train_dataloader:
                codes, kg_embed, lm_embed_centroid = batch["input_codes"], batch["input_embeds"], batch["labels"]
                ...
                if global_step % 500 == 0:
                    print(f"epoch {epoch} | train loss: {loss} | contrastive_loss {xent_loss}, back_trans_loss {back_trans_loss}")
                    if "wandb" in training_args.report_to:
                        wandb.log(
                                {
                                    "loss": loss,
            ...
            save_model_ckpt(mapper, training_args.local_rank, output_dir, distributed=False)
