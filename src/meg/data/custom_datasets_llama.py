...
class UMLSMappingDataset(Dataset):
    ...
    def tokenize_and_apply_template(self, conv):
        ...
        UMLS_INSTRUCTION = (
            ...
            "Gather useful information from the given medical concept from a knowledge graph embedding."
        )
    
        assert not self.is_inference
        _user_prompt = conv[0]["content"]
        conv = [{"role": "system", "content": UMLS_INSTRUCTION}] + conv
        tokenizer_out = self.tokenizer.apply_chat_template(
            conv, 
            add_generation_prompt=False,
            return_tensors="pt", 
...
def load_custom_dataset(data_args, model_args, tokenizer, dtype, is_train, is_inference=False):
    datasetClass = {"medqa_usmle": MedQADataset,
                    ...
                    "umls": UMLSMappingDataset}[data_args.task]
...
if __name__ == "__main__":
    ...
    dataset = UMLSMappingDataset(data_args, dtype, tokenizer, normalize_prefix, prefix_length, random_prefix, is_training=True)
