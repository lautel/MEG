...
from custom_datasets_llama import load_custom_dataset
...
from mapping_networks import MultiHeadMLPMappingNetwork, MappingType
...
class MEG(nn.Module):
    ...
    def __init__(
        ...
        # Mapping Network
        if self.config.mapping_type == MappingType.MLP:
            hidden_dim=128
    ...
    def do_forward_pass(self, input_ids, prefix, labels=None, is_training=True):
        ...
        if "umls" in self.config.task:  # Train mapping between embedding spaces
            # THIS TASK ALWAYS HAS PREFIX AND GRAPH_EMBEDS, SO NOT CONTROLLING FOR THAT
            # Right before [/INST]
            ...
            # i.e. replacing the input_embed at the position right before 
            # [/INST], which was a "double space" placeholder (id=28705 in mistral-instruct-v0.1)
